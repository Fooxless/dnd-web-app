    ...
    return (
        <div className="flex items-center justify-between px-4 py-3 sm:px-6 mt-3">
            ...
            <div className="flex flex-1 items-center justify-center">
                <div>
                    <nav className=" isolate inline-flex -space-x-px rounded-md shadow-sm bg-gray-700 border-gray-600 text-white " aria-label="Pagination">
                        <button
                            onClick={clickBack}
                    ...
                    <div>
                        <p className="text-sm mt-3 text-white justify-center flex">
                            <div>
                                Showing <span className="font-medium">{page === 1 ? (page) : ((page - 1) * 20 + 1)}
                                </span> to <span className="font-medium">{page * 20 < count ? (page * 20) : (count)}</span> of{' '}
                                <span className="font-medium">{count}</span> monsters
                            </div>
                        </p>
                    </div>
                </div>
